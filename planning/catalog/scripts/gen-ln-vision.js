...
${it.missing > 0 ? 'У песни, судя по каталогу, есть текст сверх нашего текущего — ищи внимательно, включая низ последней страницы.' : ''}
${it.missing > 0 && tp[it.key] ? 'Наш текущий текст (для сравнения):\n' + tp[it.key].stanzas.flat().join('\n') + '\nЕсли песня СКВОЗНАЯ (печатных блоков строф нет), сравни ПОДТЕКСТОВКУ под нотами с нашим текстом: фразы, которые напечатаны в нотах, но отсутствуют у нас, перепиши в verses (в порядке следования; слоговые дефисы убирай: Ge-wis-sen → Gewissen).' : ''}
${disp ? '4. Проверь спорные слова по печати (ответ — ОДНО слово как напечатано, слоговые дефисы подтекстовки убирай: Ge-wis-sen → Gewissen; если в печати слово повторено — укажи одно вхождение):\n' + disp : ''}
