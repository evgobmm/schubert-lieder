...
Работа:
1. ОТБОР в пятёрку: значительность и качество (слабая запись отсеивается и у приоритетного певца — прецедент Попп/Gretchen); возрастная структура состава (3 записи до 1990 + 2 с 1990, из них 1 с 2015 — только при наличии ДОСТОЙНЫХ, никогда в ущерб качеству). Возрастная структура влияет на СОСТАВ, не на порядок.
1а. ПОРЯДОК — строгая иерархия приоритетов (НЕ хронология и НЕ голый рейтинг критики): (1) Квастхоф — всегда №1, если его достойная запись в пятёрке; (2) затем Фишер-Дискау; (3) затем Шварцкопф; (4) затем прочие звёзды прошлого (Хоттер, Андерс, Э. Шуман, Людвиг, Попп и другие старые мастера) — между собой по качеству; (5) затем современные — между собой по качеству. У певца с несколькими версиями — учитывай состояние голоса, при прочих равных более раннюю; сходные по уровню записи внутри группы — более ранняя.
...
4. Верни JSON: d; entries (порядок = строгая иерархия приоритетов из п. 1а, первая — главная; name «Фамилия — Фамилия пианиста» латиницей, у певцов-однофамильцев инициал); reserves (2-3 с videoId если есть); composition; notes (1-3 предложения о ключевых решениях).
...
const CHUNK5 = []
for (let i = 0; i < good.length; i += 5) CHUNK5.push(good.slice(i, i + 5))
await parallel(CHUNK5.map((grp, gi) => () => agent(
  `Слей факты об альбомах из ${grp.length} досье в реестр ${ALBUMS}. ВАЖНО (контекст ограничен): реестр целиком НЕ читай — для каждого нового альбома проверяй его наличие через Bash: grep -i "<ключевые слова альбома>" ${ALBUMS} ${ROOT}/planning/youtube/albums/*.md; добавляй только отсутствующие, в конец ${ALBUMS} в формате «- **Певец — Альбом (лейбл, год)** — факты; песни: D...». Досье (бери только поле albums_facts): ${grp.map((r) => `${DATA}/${r.song.slug}.dossier.json`).join(', ')}. Больше ничего не меняй, временных файлов не создавай, субагентов не спавнь.`,
  { label: `альбомы:${gi + 1}`, phase: 'Альбомы', model: 'sonnet', effort: 'low' }
)))
