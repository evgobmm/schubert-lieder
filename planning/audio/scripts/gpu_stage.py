...
def main(list_file: str, lang: str = "de", ctc_model: str = "jonatasgrosman/wav2vec2-large-xlsr-53-german",
    ...
    # порциями по CHUNK записей: клиент Modal держит все отправленные входы в памяти до конца map (400 записей = 2,3 ГБ RAM,
    # генератор не помогает — входы вычитываются заранее); контейнер 7,8 ГБ, рядом может идти клиент другой партии
    CH = int(os.environ.get("CHUNK", "100"))
    def results():
        for i in range(0, len(todo), CH):
            part = todo[i:i + CH]; args = [(p.read_bytes(), v, p.suffix.lstrip("."), lang) for v, p in part]
            yield from stage.process.starmap(args, order_outputs=True, return_exceptions=True)
    for (v, _), res in zip(todo, results()):
