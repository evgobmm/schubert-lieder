#!/bin/bash
# Прогон wh_pipeline без записи в app/: test_run.sh <spec.json> <outdir> vid…
# Данные: $SP/align (Whisper-транскрипты wh_<vid>.json, эмиссии em_*.pt), $SP/audio (стемы <vid>_voc.wav); SP — каталог над каталогом скриптов.
# Печатает маршрут каждой записи и, если в app/ уже есть файл, — отличия маршрута, сдвиги начал >0.3 с по общим проходам и варианты.
set -e; SCRIPTS=$(dirname "$(realpath "$0")"); SP=$(cd "$SCRIPTS/.."; pwd); PY=$SP/align/.venv/bin/python
SPEC=$(realpath "$1"); OUT=$(realpath -m "$2"); shift 2; mkdir -p "$OUT"
...
for v in "$@"; do
  ...
  (cd "$OUT/wh_$v" && OUT_DIR=$OUT DEBUG_PASSES=${DEBUG_PASSES:-} nice -n 15 $PY $SCRIPTS/wh_pipeline.py "$SPEC" $v $SP/align/${WHVAR:-wh}_$v.json $SP/audio/${v}_voc.wav $SP/align/em_mms_$v.pt $SP/align/em_de_$v.pt 2>&1 | grep -a -v Warn | grep -a -E "спето|отброшено|хвост|голова|купюра|подтверждено|проход |дополнен|консенсус предлагает|вне консенсуса|Traceback|Error|File " | cut -c1-220)
