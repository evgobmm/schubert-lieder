...
for lang in $(cut -d' ' -f2 "$SP/songs/batch.txt" | sort -u); do
  ...
  for attempt in 1 2 3; do   # при обрыве соединения (DNS, сеть) — повтор: готовые записи пропускаются
    (cd "$SCRIPTS" && $SP/align/.venv/bin/modal run gpu_stage.py::main --list-file "$SP/songs/vids_$lang.txt" --lang "$lang" --ctc-model "$CTC" --indir "$SP/full" --outdir "$SP/align" --audiodir "$SP/audio" 2>&1 | grep -a -E "на GPU|готово за|ОШИБКА|Error|disabled" | cut -c1-160)
    left=$(for v in $(cat "$SP/songs/vids_$lang.txt"); do [ -s "$SP/align/wh_$v.json" ] || echo x; done | wc -l); echo "  осталось без выходов: $left"; [ "$left" = "0" ] && break
  done
done
