...
# один коммит за раз: параллельные цепочки партий сериализуются замком (ожидание через pgrep ловило собственную обёртку bash -c и висело вечно)
exec 9>"${LOCK:-/tmp/schubert-publish.lock}"; flock 9
...
if [ -n "$2" ]; then
  ...
  # удалённые (перенесённые в held) — одним вызовом ls-files: 212 вызовов по песне занимали ~3 мин на overlay-диске
  { for p in $(cut -d' ' -f1 "$2"); do ls $T/$p-*.json 2>/dev/null || true; done; git ls-files --deleted -- $T | grep -F -f <(cut -d' ' -f1 "$2" | sed "s#^#$T/#; s#\$#-#") || true; } | sort -u | xargs -r git add -A --
else git add -A $T; fi
