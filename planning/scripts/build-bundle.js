...
if (stage === 'dict') {
  ...
  if (pre) out += sec('ПРЕДЗАГРУЖЕННЫЕ ВЫПИСКИ (DWDS: лемма по редиректу и обзор значений; Grimm DWB: начало статьи; in_cache=true — лемма уже в кэше, новой записи не нужно, только map)', pre.map((r) => [`• ${r.form}${r.note ? ' — ' + r.note : ''}`, r.dwds && r.dwds.lemma ? `  DWDS: лемма «${r.dwds.lemma}» ${r.dwds.url}${r.in_cache ? ' — В КЭШЕ' : ''}` : '  DWDS: не найдено', r.dwds && r.dwds.head ? `  DWDS обзор: ${r.dwds.head.slice(0, 700)}` : '', r.dwds && r.dwds.etymology ? `  DWDS этимология: ${r.dwds.etymology.slice(0, 300)}` : '', r.dwb && r.dwb.head ? `  DWB (${r.dwb.url}): ${r.dwb.head.slice(0, 1100)}` : '  DWB: нет статьи'].filter(Boolean).join('\n')).join('\n\n'));
